...
class PrivateRoomAgent(Agent):
    ...
    class SendRoomDataExchangeRequestBehaviour(CyclicBehaviour):
        async def run(self):
            msg = PrivateRoomAgent.prepare_room_data_exchange_request(self, 20, 'another_room_agent')
            await self.send(msg)
    ...
    class SendEnergyUsageInformBehaviour(CyclicBehaviour):
        async def run(self):
            ...
            msg = PrivateRoomAgent.prepare_energy_usage_inform(self, energy, 'energy_agent')
            await self.send(msg)

    class SendOutdoorTemperatureRequestBehaviour(CyclicBehaviour):
        async def run(self):
            msg = PrivateRoomAgent.prepare_outdoor_temperature_request(self, 'outdoor_agent')
            await self.send(msg)
            #czekanie na inform
